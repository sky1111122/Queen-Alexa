...
function setupUI() {
	...
	var css = `
        ...
        #modmenuclosebtn {
            ...
            margin-top: 84px;
            margin-right: 117px;
            background: url("https://imgur.com/gallery/yy-DVAwRLR");
...
function setupModMenu() {
	...
	if (!window.onMobile) {
		const press4Btn = createModBtn("press4Btn", "on", "press4On", "4 key respawn");
	...
	const spamLastEmoteBtn = createModBtn("spamLastEmoteBtn", "on", "spamLastEmoteOn", "Spam Recent Emoji");
	const spamEmojiBtn = createModBtn("spamEmojiBtn", "on", "spamEmojisOn", "Spam All Emojis");
...
setInterval(() => {
	if (document.querySelector("#modmenubtn").hidden == true && document.querySelector("#modmenu").hidden == false) {
		document.querySelector("#modmenubtn").hidden = "false";
	}
}, 1000);
...
var cribl = [
	"https://i.imgur.com/lhQfNGy_d.webp?maxwidth=760&fidelity=grand",
